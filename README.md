...
- **Методы класса**:
  ...
  - `bindEvents(onBuy: Function, onRemove: Function): void`  
    Привязывает обработчики событий к кнопкам «Купить» и «Удалить».
...
- **Поля класса**:
  ...
  - `totalElement` (HTMLElement) — элемент для отображения общей стоимости товаров в корзине.
  - `submitButton` (HTMLElement) — кнопка для оформления заказа.

- **Методы класса**:

  - `renderCart(items: HTMLElement[]): void`  
    Заменяет содержимое корзины на переданные элементы. Использует метод `replaceChildren` для вставки.
