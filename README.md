...
- **Поля класса**:
  ...
  - `products: Array<Product>` — массив объектов товаров. Каждый объект `Product` представляет данные одного товара.

- **Методы класса**:

  - `setProducts(products: Array<Product>): void`  
    Принимает массив объектов товаров и сохраняет его в поле `products`.

  - `getProducts(): Array<Product>`  
    Возвращает текущий массив объектов товаров из поля `products`.
...
- **Поля класса**:
  ...
  - `items: Array<CartItem>` — массив объектов корзины. Каждый объект `CartItem` содержит данные о товаре и его количестве.
...
- **Конструктор**:  
  Принимает начальные данные о заказе и инициализирует поля класса.
...
- **Методы класса**:
  ...
  - `validateOrder(): Array<string>`  
    Проверяет корректность данных заказа (адрес, способ оплаты). Возвращает массив ошибок (или пустой массив, если всё корректно).
...
- **Методы класса**:
  ...
  - `bindCardEvents(cards: HTMLElement[]): void`  
...
- **Методы класса**:
  ...
  - `validate(): boolean`  
    Проверяет корректность заполнения полей. Возвращает `true`, если все поля заполнены, иначе — `false`.
...
- **Поля класса**:
  ...
  - `container` (HTMLElement) — элемент, в котором отображается форма.
  - `email` (string) — введённый адрес электронной почты.
...
- **Назначение**:  
  Отвечает за отображение корзины товаров. Управляет контейнером корзины и рендерит список карточек товаров (HTML-элементы).

- **Конструктор**:  
  Принимает следующие параметры:

  - `container` (HTMLElement) — DOM-элемент, куда будет рендериться корзина.
...
- **Методы класса**:
  ...
  - `renderCart(items: HTMLElement[]): void`  
    Очищает `container` и добавляет в него список карточек товаров, переданных как массив HTML-элементов.
